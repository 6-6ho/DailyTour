...
def exchange_score():
    ...
    # 모든 국가에 대한 초기 점수 계산
    country_info['SEARCH_SCORE'] = country_info['RATE_DIV'].apply(lambda x: calculate_rate_div_score(x, min_div, max_div, 5))
    ...
    for idx, row in country_info_without_TR.iterrows():
        country_info.at[idx, 'SEARCH_SCORE'] = calculate_rate_div_score(row['RATE_DIV'], min_div_without_TR, max_div_without_TR, 4.5)
    ...
    for idx, row in country_info_without_TR_MN.iterrows():
        country_info.at[idx, 'SEARCH_SCORE'] = calculate_rate_div_score(row['RATE_DIV'], min_div_without_TR_MN, max_div_without_TR_MN, 4.0)
    

    result_df = country_info[['CNT_CODE', 'CURRENCY', 'EX_RATE', 'EX_AVG', 'SEARCH_SCORE']]
    print(result_df)
    #country_info[['CNT_CODE', 'CURRENCY', 'EX_RATE', 'EX_AVG', 'SEARCH_SCORE']].to_csv(file_path, index=False, encoding='utf-8')
